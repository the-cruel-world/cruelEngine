...
RenderSession::RenderSession(Instance &instance, LogicalDevice &device,
    ...
    : instance{instance}, device{device}, session_properties{
                                              session_properties} {
  ...
  std::cout << "[Rendersession] graphic queue info: "
            << graphic_queue->get_family_index() << graphic_queue->get_index()
            << std::endl;
  std::cout << "[Rendersession] present queue info: "
            << present_queue->get_family_index() << present_queue->get_index()
            << std::endl;
...
void RenderSession::draw() {
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    commandBuffers[i].get().begin_renderpass(
...
void RenderSession::render_frame() {
  ...
  if (imagesInFlight[image_index] != VK_NULL_HANDLE) {
    vkWaitForFences(device.get_handle(), 1, &imagesInFlight[image_index],
                    VK_TRUE, UINT64_MAX);
  }
  ...
  submitInfo.pCommandBuffers =
      &(commandBuffers[image_index].get().get_handle());
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    if (vkCreateSemaphore(device.get_handle(), &semaphoreInfo, nullptr,
      ...
      throw std::runtime_error(
          "createSemaphores: failed to create semaphores!");
  }
}

void RenderSession::destroySemaphores() {
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    vkDestroySemaphore(device.get_handle(), renderFinishedSemaphores[i],
                       nullptr);
