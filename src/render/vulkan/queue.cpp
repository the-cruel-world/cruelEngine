...
Queue::Queue(LogicalDevice &_device, const u32 _family_index, const u32 _index,
...
Queue::Queue(Queue &&other)
    : device{other.device}, handle{other.handle},
      family_index{other.family_index}, index{other.index},
...
VkResult Queue::submit(const std::vector<VkSubmitInfo> &submit_infos,
                       VkFence fence) {
  return vkQueueSubmit(handle, u32(submit_infos.size()), submit_infos.data(),
                       fence);
}
...
VkResult Queue::wait_idle() { return vkQueueWaitIdle(handle); }
} // namespace cruelRender
} // namespace cruelEngine