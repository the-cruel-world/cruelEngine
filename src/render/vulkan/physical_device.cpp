...
bool isPhysicalDeviceSuitable(
  ...
  for (const auto &requiredExtension : requiredExtensions) {
    ...
    if (find(supportedExtensions.begin(), supportedExtensions.end(),
             requiredExtension) == supportedExtensions.end()) {
      extensionSupported = false;
      break;
    }
  }
  if (extensionSupported == false) {
    std::cerr << "at least one extension is not supported by device"
              << std::endl;
    return false;
  }
...
PhysicalDevice::~PhysicalDevice() { std::cout << "gpu destroied" << std::endl; }

const u32
PhysicalDevice::getMemoryTypeIndex(const u32 &filter,
