...
namespace cruelRender {
        ...
        physicalDevice(_physicalDevice), layerNames(_layerNames), requiredExtensions(_requiredExtensions), flags(_flags)
    {
        ...
        createDevice();
        // std::cout << "[logical] device created. time: " << (clock() - start_time) * 1e-5  << "ms" << std::endl;
        // start_time = clock();
        query_queues();
        // std::cout << "[logical] queue pool created. time: " << (clock() - start_time) * 1e-5  << "ms" << std::endl;
    ...
    void LogicalDevice::createDevice()
    {
        ...
        vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice.get_handle(), &queueFamilyCount, nullptr);
        std::vector<VkQueueFamilyProperties> queueFamilies(queueFamilyCount);
        vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice.get_handle(), &queueFamilyCount, queueFamilies.data());
        ...
        VK_CHECK_RESULT(vkCreateDevice(physicalDevice.get_handle(), &deviceCreateInfo, nullptr, &handle));
    }
