...
namespace VulkanContext {
    ...
    void PipelineLayout::createPipelineLayout()
    {
        ...
        if (vkCreatePipelineLayout(device.get_handle(), &pipelineLayoutInfo, nullptr, &handle) != VK_SUCCESS) {
            ...
            : Pipeline{device}
    {
        ...
        if (shader_module.get_shader_stage() != VK_SHADER_STAGE_COMPUTE_BIT) {
            throw std::runtime_error("Creating ComputePipeline: Compute shader module required.");
        }
        ...
        stage.module = createShaderModule(device.get_handle(), shader_module.get_source());
        ...
        VK_CHECK_RESULT (vkCreateComputePipelines(device.get_handle(), pipeline_cache, 1, &create_info, nullptr, &handle));
            ...
            : Pipeline{device}
    {
        ...
        for (auto &shader_module : pipeline_state.get_pipeline_layout()->get_shader_modules())
        {
            ...
            stage_create_info.module = createShaderModule(device.get_handle(), shader_module.get_source());
	    ...
	    vertex_input_state.vertexAttributeDescriptionCount = (u32)(pipeline_state.get_vertex_input_state().attributes.size());
        vertex_input_state.pVertexBindingDescriptions    = pipeline_state.get_vertex_input_state().bindings.data();
        vertex_input_state.vertexBindingDescriptionCount = (u32)(pipeline_state.get_vertex_input_state().bindings.size());
        ...
        VK_CHECK_RESULT (vkCreateGraphicsPipelines(device.get_handle(), pipeline_cache, 1, &create_info, nullptr, &handle));
