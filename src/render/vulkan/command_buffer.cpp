...
namespace VulkanContext {
        ...
        level(_level)
    {
        ...
        VK_CHECK_RESULT(vkAllocateCommandBuffers(commandPool.get_device().get_handle(), &allocate_info, &handle));
    }
    ...
    CommandBuffer::~CommandBuffer()
    {
        ...
        {
            vkFreeCommandBuffers(commandPool.get_device().get_handle(), commandPool.get_handle(), 1, &handle);
        }
    }
    ...
    void CommandBuffer::endOneTime()
    {
        ...
        vkQueueSubmit(commandPool.get_device().get_suitable_graphics_queue(0).get_handle(), 1, &submitInfo, VK_NULL_HANDLE);
        vkQueueWaitIdle(commandPool.get_device().get_suitable_graphics_queue(0).get_handle());
