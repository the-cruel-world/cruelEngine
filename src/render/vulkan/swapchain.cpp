...
VkExtent2D choose_extent(VkExtent2D requestExtent, VkExtent2D minExtent,
                         VkExtent2D maxExtent) {
  requestExtent.height = std::min(
      std::max(requestExtent.height, minExtent.height), maxExtent.height);
  requestExtent.width =
      std::min(std::max(requestExtent.width, minExtent.width), maxExtent.width);
...
VkSurfaceFormatKHR
choose_surface_format(VkSurfaceFormatKHR request,
      ...
      std::find_if(available.begin(), available.end(),
                   [&request](const VkSurfaceFormatKHR &format) {
                     return (format.format == request.format &&
                             format.colorSpace == request.colorSpace)
                                ? true
                                : false;
                   });
...
choose_transform(VkSurfaceTransformFlagBitsKHR request,
...
choose_composite_alpha(VkCompositeAlphaFlagBitsKHR request,
...
VkImageUsageFlags
composite_image_flags(std::vector<VkImageUsageFlagBits> &image_usage_flags) {
...
Swapchain::Swapchain(LogicalDevice &_device, const VkSurfaceKHR &_surface,
    ...
    : Swapchain{*this,       _device,    _surface,    _extent,
                _imageCount, _transform, _presentMode} {}

Swapchain::Swapchain(Swapchain &old_swapchain, LogicalDevice &_device,
  ...
  vkGetPhysicalDeviceSurfaceFormatsKHR(device.get_physicalDevice().get_handle(),
                                       surface, &formatCount, nullptr);
  if (formatCount != 0) {
    formats.resize(formatCount);
    vkGetPhysicalDeviceSurfaceFormatsKHR(
        device.get_physicalDevice().get_handle(), surface, &formatCount,
        formats.data());
  }
  ...
  vkGetPhysicalDeviceSurfacePresentModesKHR(
      device.get_physicalDevice().get_handle(), surface, &presentModeCount,
      nullptr);
  ...
  properties.image_count = choose_image_count(
      _imageCount, capabilities.minImageCount, capabilities.maxImageCount);
  properties.extent = choose_extent(_extent, capabilities.minImageExtent,
                                    capabilities.maxImageExtent);
  properties.present_mode = choose_present_mode(_presentMode, presentModes,
                                                present_mode_priority_list);
  properties.surface_format = choose_surface_format(
      properties.surface_format, formats, surface_format_priority_list);

  properties.transform =
      choose_transform(_transform, capabilities.supportedTransforms,
                       capabilities.currentTransform);
  properties.composite_alpha = choose_composite_alpha(
      VK_COMPOSITE_ALPHA_INHERIT_BIT_KHR, capabilities.supportedCompositeAlpha);
  ...
  VK_CHECK_RESULT(vkCreateSwapchainKHR(device.get_handle(), &create_info,
                                       nullptr, &handle));

  u32 available_images = 0;
  VK_CHECK_RESULT(vkGetSwapchainImagesKHR(device.get_handle(), handle,
                                          &available_images, nullptr));

  images.resize(available_images);
  VK_CHECK_RESULT(vkGetSwapchainImagesKHR(device.get_handle(), handle,
                                          &available_images, images.data()));
  ...
  for (size_t i = 0; i < images.size(); i++) {
    ...
    VK_CHECK_RESULT(vkCreateImageView(device.get_handle(), &createInfo, nullptr,
                                      &imageViews[i]));
  }
}

Swapchain::Swapchain(Swapchain &old_swapchain, const VkExtent2D _extent)
    : Swapchain(old_swapchain, old_swapchain.device, old_swapchain.surface,
                _extent, old_swapchain.properties.image_count,
                old_swapchain.properties.transform,
