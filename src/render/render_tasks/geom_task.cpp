...
void GeomTask::draw(cruelRender::CommandBuffer &commandBuffer, int index)
{
    ...
    vkCmdBindIndexBuffer(commandBuffer.get_handle(), index_buffer->get_handle(), 0,
    ...
    vkCmdDrawIndexed(commandBuffer.get_handle(), object->get_mesh().get_triangles().size(), 1, 0, 0,
                     0);
}
...
void GeomTask::render()
{
    ...
    ubo.view = glm::lookAt(glm::normalize(glm::vec3(1.0f, -1.0f, -2.0f)) * glm::vec3(3.0),
...
void GeomTask::prepare_pipeline()
{
    ...
    vertex_input_state.attributes.push_back(cruelScene::Vertex::getAttributeDescriptions()[0]);
    vertex_input_state.attributes.push_back(cruelScene::Vertex::getAttributeDescriptions()[1]);

    vertex_input_state.bindings.push_back(cruelScene::Vertex::getBindingDescription());
...
void GeomTask::prepare_pipeline(VkPipelineCache pipeline_cache, PipelineStatus &pipeline_state)
{
    pipeline =
...
void GeomTask::prepare_assets()
{
    ...
    descriptor_pool = std::make_unique<cruelRender::DescriptorPool>(session.get_device(), poolSize);
    ...
    descriptor_set_layout =
        std::make_unique<cruelRender::DescriptorSetLayout>(session.get_device(), bindings);
