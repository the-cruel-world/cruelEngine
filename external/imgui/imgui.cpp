...
ImGuiStyle::ImGuiStyle() {
  ...
  DisplayWindowPadding = ImVec2(
      20, 20); // Window position are clamped to be visible within the display
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height) {
  ...
  window->DC.CursorPosPrevLine.y =
      ...
      line_height; // Setting those fields so that SetScrollHere() can properly
...
bool ImGuiListClipper::Step() {
  ...
  if (StepNo ==
      0) // Step 0: the clipper let you process the first element, regardless of
...
bool ImGui::ItemAdd(const ImRect &bb, ImGuiID id, const ImRect *nav_bb_arg) {
  ...
  if (id != 0) {
    ...
    //  unfortunately, but it is still limited to one window.
    ...
    //      aka maximum once a frame. We could early out with "if (is_clipped &&
...
bool ImGui::IsItemHovered(ImGuiHoveredFlags flags) {
  ...
  // RootWindow instead. I believe it is correct to NOT test for RootWindow but
  // this leaves us unable to use IsItemHovered() after EndChild() itself. Until
  // a solution is found I believe reverting to the test from 2017/09/27 is safe
  // since this was the test that has been running for a long while.
  ...
  if (tab_stop && (g.ActiveId == id) &&
    ...
    window->FocusIdxTabRequestNext =
        ...
        (g.IO.KeyShift
             ...
             : +1); // Modulo on index will be applied at the end of frame once
  ...
  for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++) {
    ...
    if (g.IO.MouseClicked[i]) {
      ...
      g.IO.MouseDragMaxDistanceSqr[i] = ImMax(
          g.IO.MouseDragMaxDistanceSqr[i], ImLengthSqr(delta_from_click_pos));
...
void ImGui::UpdateMouseWheel() {
  ...
  // a chance to scroll its parent (unless either ImGuiWindowFlags_NoInputs or
...
void ImGui::UpdateHoveredWindowAndCaptureFlags() {
  ...
  // info to imgui, false = dispatch mouse info to imgui + app)
...
static void AddDrawListToDrawData(ImVector<ImDrawList *> *out_list,
  ...
  // manually: A) Make sure you are coarse clipping, because ImDrawList let all
  // your vertices pass. You can use the Metrics window to inspect draw list
  // contents. B) If you need/want meshes with more than 64K vertices, uncomment
  // the '#define ImDrawIdx unsigned int' line in imconfig.h to set the index
  // size to 4 bytes.
...
void ImGui::EndFrame() {
  ...
  IM_ASSERT(g.CurrentWindowStack.Size ==
            1); // Mismatched Begin()/End() calls, did you forget to call end on
  ...
  for (int n = 0; n < IM_ARRAYSIZE(windows_to_render_front_most); n++)
    if (windows_to_render_front_most[n] &&
        IsWindowActiveAndVisible(
            windows_to_render_front_most[n])) // NavWindowingTarget is always
...
ImVec2 ImGui::GetMouseDragDelta(int button, float lock_threshold) {
  ...
  if (g.IO.MouseDown[button])
    if (g.IO.MouseDragMaxDistanceSqr[button] >= lock_threshold * lock_threshold)
      return g.IO.MousePos -
             g.IO.MouseClickedPos[button]; // Assume we can only get active with
                                           // left-mouse button (at the moment).
...
bool ImGui::IsItemDeactivated() {
  ...
  return (g.ActiveIdPreviousFrame == window->DC.LastItemId &&
          g.ActiveIdPreviousFrame != 0 && g.ActiveId != window->DC.LastItemId);
}

bool ImGui::IsItemDeactivatedAfterEdit() {
  ImGuiContext &g = *GImGui;
  return IsItemDeactivated() && (g.ActiveIdPreviousFrameHasBeenEdited ||
...
static bool ImGui::BeginChildEx(const char *name, ImGuiID id,
  ...
  char title[256];
  if (name)
    ImFormatString(title, IM_ARRAYSIZE(title), "%s/%s", parent_window->Name,
                   name);
  else
  ...
  if (g.NavActivateId == id && !(flags & ImGuiWindowFlags_NavFlattened) &&
    ...
    SetActiveID(id + 1, child_window); // Steal ActiveId with a dummy id so that
...
static ImGuiWindow *CreateNewWindow(const char *name, ImVec2 size,
  ...
  window->DC.CursorMaxPos = window->Pos; // So first call to CalcSizeContents()
                                         // doesn't return crazy values
...
static ImVec2 CalcSizeAutoFit(ImGuiWindow *window,
  ...
  if (window->Flags & ImGuiWindowFlags_Tooltip) {
    ...
    ImVec2 size_auto_fit =
        ImClamp(size_contents, size_min,
                ImMax(size_min,
...
static void ImGui::UpdateManualResize(ImGuiWindow *window,
  ...
  for (int border_n = 0; border_n < resize_border_count; border_n++) {
    const float BORDER_SIZE = 5.0f; // FIXME: Only works _inside_ window because
                                    // of HoveredWindow check.
    const float BORDER_APPEAR_TIMER = 0.05f; // Reduce visual noise
    bool hovered, held;
    ImRect border_rect =
        GetResizeBorderRect(window, border_n, grip_hover_size, BORDER_SIZE);
...
bool ImGui::Begin(const char *name, bool *p_open, ImGuiWindowFlags flags) {
  ...
  if (first_begin_of_the_frame) {
    ...
    // we recycle the windows) We reset Size/SizeContents for reappearing
    ...
    if (!(flags & ImGuiWindowFlags_ChildWindow)) {
      if (!window_pos_set_by_api && window->AutoFitFramesX <= 0 &&
          ...
          g.IO.DisplaySize.y >
              0.0f) // Ignore zero-sized display explicitly to avoid losing
    ...
    if (dim_bg_for_window_list && window == g.NavWindowingTargetAnim) {
      ...
      if (!bb.Contains(viewport_rect)) // Avoid drawing if the window covers all
                                       // the viewport anyway
    ...
    if (window->Collapsed) {
      ...
      if (border_held != -1) {
        ImRect border =
            GetResizeBorderRect(window, border_held, grip_draw_size, 0.0f);
    ...
    // Update various regions. Variables they depends on are set above in this
    // function.
      ...
      if (!(flags & ImGuiWindowFlags_NoCollapse))
        if (CollapseButton(window->GetID("#COLLAPSE"), window->Pos))
          window->WantCollapseToggle =
              true; // Defer collapsing to next frame as we are too far in the
                    // Begin() function
    ...
    */
    ...
    // will end up with an inverted (~null) clipping rectangle which is the
    // correct behavior.
    ...
    // data. It is a standard behavior (to allow creation of context menus on
    // title bar only, etc.).
  ...
  if (flags & ImGuiWindowFlags_ChildWindow) {
    ...
    // collapse them because they have no title bar).
  ...
  // arbitrary and inconsistent but has been there for a long while (may remove
  // at some point)
  if (style.Alpha <= 0.0f)
    window->HiddenFramesRegular = 1;
  ...
  window->Hidden =
      (window->HiddenFramesRegular > 0) || (window->HiddenFramesForResize);
...
void ImGui::SetWindowScrollX(ImGuiWindow *window, float new_scroll_x) {
  window->DC.CursorMaxPos.x +=
      window->Scroll.x; // SizeContents is generally computed based on
                        // CursorMaxPos which is affected by scroll position, so
                        // we need to apply our change to it.
...
void ImGui::SetNextWindowContentSize(const ImVec2 &size) {
  ...
  g.NextWindowData.ContentSizeVal =
      size; // In Begin() we will add the size of window decorations (title bar,
...
void ImGui::EndGroup() {
  ...
  if (group_data.AdvanceCursor) {
    window->DC.CurrentLineTextBaseOffset = ImMax(
        ...
        group_data
            .BackupCurrentLineTextBaseOffset); // FIXME: Incorrect, we should
...
bool ImGui::BeginPopupContextItem(const char *str_id, int mouse_button) {
  ...
  ImGuiID id =
      str_id ? window->GetID(str_id)
             : window->DC.LastItemId; // If user hasn't passed an ID, we can use
...
ImVec2 ImGui::FindBestWindowPosForPopup(ImGuiWindow *window) {
  ...
  if (window->Flags & ImGuiWindowFlags_ChildMenu) {
    ...
    // and then our FindBestWindowPosForPopup() function will move the new menu
    // outside the parent bounds. This is how we end up with child menus
    ...
    float horizontal_overlap =
        g.Style.ItemSpacing.x; // We want some overlap to convey the relative
                               // depth of each menu (currently the amount of
                               // overlap is hard-coded to style.ItemSpacing.x).
    ImRect r_avoid;
    if (parent_window->DC.MenuBarAppending)
      r_avoid = ImRect(-FLT_MAX,
  ...
  if (window->Flags & ImGuiWindowFlags_Tooltip) {
    ...
    if (window->AutoPosLastDirection == ImGuiDir_None)
      pos = ref_pos +
            ImVec2(2, 2); // If there's not enough room, for tooltip we prefer
...
static bool NavScoreItem(ImGuiNavMoveResult *result, ImRect cand) {
  ...
  const ImRect &curr =
      g.NavScoringRectScreen; // Current modified source rect (NB: we've applied
...
static void ImGui::NavProcessItem(ImGuiWindow *window, const ImRect &nav_bb,
  ...
  if (g.NavId == id) {
    g.NavWindow = window; // Always refresh g.NavWindow, because some operations
...
void ImGui::NavMoveRequestTryWrapping(ImGuiWindow *window,
  ...
  if (g.NavMoveDir == ImGuiDir_Left &&
    ...
    bb_rel.Min.x = bb_rel.Max.x =
        ImMax(window->SizeFull.x, window->SizeContents.x) - window->Scroll.x;
  ...
  if (g.NavMoveDir == ImGuiDir_Up &&
    ...
    bb_rel.Min.y = bb_rel.Max.y =
        ImMax(window->SizeFull.y, window->SizeContents.y) - window->Scroll.y;
...
static ImVec2 ImGui::NavCalcPreferredRefPos() {
  ...
  return ImFloor(ImClamp(
      ...
      visible_rect.Max)); // ImFloor() is important because non-integer mouse
...
float ImGui::GetNavInputAmount(ImGuiNavInput n, ImGuiInputReadMode mode) {
  ...
  if (t < 0.0f &&
      mode == ImGuiInputReadMode_Released) // Return 1.0f when just released, no
...
static void ImGui::NavUpdate() {
  ...
  // Set input source as Gamepad when buttons are pressed before we map Keyboard
  // (some features differs when used with Gamepad vs Keyboard)
...
#undef NAV_MAP_KEY
  ...
  if (g.NavMoveRequestForward == ImGuiNavForward_None) {
    ...
    // the window) (Preserve g.NavMoveRequestFlags, g.NavMoveClipDir which were
  ...
  IM_ASSERT(!g.NavScoringRectScreen
...
static void ImGui::NavUpdateMoveResult() {
  ...
  if (g.NavLayer == 0) {
    ...
    // Estimate upcoming scroll so we can offset our result position so mouse
    // position can be applied immediately after in NavUpdate()
    ImVec2 next_scroll =
    ...
    // could/should technically recurse back the whole the parent hierarchy).
    ...
    if ((page_up_held && !page_down_held) ||
      ...
      if (window->DC.NavLayerActiveMask == 0x00 && window->DC.NavHasScroll) {
        ...
        const float page_offset_y = ImMax(
            0.0f, window->InnerClipRect.GetHeight() -
...
static void ImGui::NavUpdateWindowing() {
  ...
  if (start_windowing_with_gamepad || start_windowing_with_keyboard)
    if (ImGuiWindow *window =
            g.NavWindow
                ? g.NavWindow
                : FindWindowNavFocusable(g.Windows.Size - 1, -INT_MAX, -1)) {
...
void ImGui::NextColumn() {
  ...
  if (++columns->Current < columns->Count) {
    ...
    window->DC.ColumnsOffset.x = GetColumnOffset(columns->Current) -
                                 window->DC.Indent.x + g.Style.ItemSpacing.x;
...
void ImGui::SetColumnOffset(int column_index, float offset) {
  ...
  if (!(columns->Flags & ImGuiColumnsFlags_NoForceWithinWindow))
    offset = ImMin(offset, columns->MaxX - g.Style.ColumnsMinSpacing *
...
void ImGui::EndColumns() {
  ...
  if (!(columns->Flags & ImGuiColumnsFlags_GrowParentContentsSize))
    window->DC.CursorMaxPos.x =
        columns->StartMaxPosX; // Restore cursor max pos, as columns don't grow
                               // parent
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags) {
  ...
  if (!(flags & ImGuiDragDropFlags_SourceExtern)) {
    ...
    if (source_id == 0) {
      ...
      // assigned ID, e.g. Text(), Image() We build a throwaway ID based on
      // current ID stack + relative AABB of items in window. THE IDENTIFIER
      // WON'T SURVIVE ANY REPOSITIONING OF THE WIDGET, so if your widget moves
      // your dragging operation will be canceled. We don't need to
      // maintain/call ClearActiveID() as releasing the button will early out
      // this function and trigger !ActiveIdIsAlive.
...
static void SettingsHandlerWindow_WriteAll(ImGuiContext *imgui_ctx,
  ...
  for (int i = 0; i != g.Windows.Size; i++) {
    ...
    ImGuiWindowSettings *settings =
        (window->SettingsIdx != -1) ? &g.SettingsWindows[window->SettingsIdx]
...
void ImGui::ShowMetricsWindow(bool *p_open) {
  ...
  struct Funcs {
    static void NodeDrawList(ImGuiWindow *window, ImDrawList *draw_list,
      ...
      ImDrawList *overlay_draw_list =
          GetOverlayDrawList(); // Render additional visuals into the top-most
                                // draw list
        ...
        while (clipper.Step())
          for (int prim = clipper.DisplayStart,
            ...
            for (int n = 0; n < 3; n++, vtx_i++) {
              ImDrawVert &v =
                  draw_list->VtxBuffer[idx_buffer ? idx_buffer[vtx_i] : vtx_i];
  ...
  if (ImGui::TreeNode("Internal state")) {
    const char *input_source_names[] = {"None", "Mouse", "Nav", "NavKeyboard",
                                        "NavGamepad"};
    ...
    ImGui::Text(
        ...
        g.HoveredIdAllowOverlap); // Data is "in-flight" so depending on when
                                  // the Metrics window is called we may see
