...
#define IM_ARRAYSIZE(_ARR)                                                     \
  ((int)(sizeof(_ARR) / sizeof(*_ARR))) // Size of a static C-style array. Don't
                                        // use on pointers!
...
BeginChild(const char *str_id, const ImVec2 &size = ImVec2(0, 0),
           ...
           ImGuiWindowFlags flags =
               0); // Begin a scrolling region. size==0.0f: use remaining window
                   // size, size<0.0f: use remaining window size minus
...
IMGUI_API void
SetWindowFontScale(float scale); // set font scale. Adjust IO.FontGlobalScale if
                                 // you want to scale all windows
...
IMGUI_API void
SetWindowFocus(const char *name); // set named window to be focused /
                                  // front-most. use NULL to remove focus.
...
IMGUI_API void
Indent(float indent_w = 0.0f); // move content position toward the right, by
                               // style.IndentSpacing or indent_w if != 0
IMGUI_API void
Unindent(float indent_w = 0.0f); // move content position back to the left, by
                                 // style.IndentSpacing or indent_w if != 0
IMGUI_API void
...
IMGUI_API void SetCursorScreenPos(
    const ImVec2 &screen_pos); // cursor position in absolute screen coordinates
                               // [0..io.DisplaySize]
IMGUI_API void
...
BeginPopupModal(const char *name, bool *p_open = NULL,
                ImGuiWindowFlags flags =
                    0); // modal dialog (regular window with title bar, block
                        // interactions behind the modal window, can't close the
...
IMGUI_API bool
IsMouseDown(int button); // is mouse button held (0=left, 1=right, 2=middle)
...
IMGUI_API void CaptureKeyboardFromApp(
    bool capture =
        true); // manually override io.WantCaptureKeyboard flag next frame (said
               // flag is entirely left for your application to handle). e.g.
...
enum ImGuiInputTextFlags_ {
  ...
  ImGuiInputTextFlags_CallbackResize =
      1 << 18, // Allow buffer capacity resize + notify when the string wants to
...
struct ImGuiStyle {
  ...
  ImVec2 ButtonTextAlign; // Alignment of button text when button is larger than
                          // text. Defaults to (0.5f,0.5f) for
...
struct ImGuiIO {
  ...
  ImGuiBackendFlags
      BackendFlags; // = 0                  // Set ImGuiBackendFlags_ enum. Set
                    // by imgui_impl_xxx files or custom back-end to communicate
                    // features supported by the back-end.
  ...
  float
      MouseDoubleClickMaxDist; // = 6.0f               // Distance threshold to
                               // stay in to validate a double-click, in pixels.
  ...
  bool
      ConfigMacOSXBehaviors; // = defined(__APPLE__) // OS X style: Text editing
                             ...
                             // in lists uses Cmd/Super instead of Ctrl (was
                             // called io.OptMacOSXBehaviors prior to 1.63)
  ...
  bool MouseDown[5]; // Mouse buttons: 0=left, 1=right, 2=middle + extras. ImGui
  ...
  float
      MouseWheelH; // Mouse wheel Horizontal. Most users don't have a mouse with
                   // an horizontal wheel, may not be filled by all back-ends.
  bool KeyCtrl;    // Keyboard modifier pressed: Control
  bool KeyShift;   // Keyboard modifier pressed: Shift
  bool KeyAlt;     // Keyboard modifier pressed: Alt
  bool KeySuper;   // Keyboard modifier pressed: Cmd/Super/Windows
  bool KeysDown[512]; // Keyboard keys that are pressed (ideally left in the
                      // "native" order your engine has access to keyboard keys,
  ...
  float NavInputs[ImGuiNavInput_COUNT]; // Gamepad inputs (keyboard keys will be
                                        // auto-mapped and be written here by
                                        // ImGui::NewFrame, all values will be
                                        // cleared back to zero in
                                        // ImGui::EndFrame)
  ...
  IMGUI_API void AddInputCharactersUTF8(
      const char *utf8_chars); // Add new characters into InputCharacters[] from
                               // an UTF-8 string
  ...
  bool WantCaptureMouse; // When io.WantCaptureMouse is true, imgui will use the
                         // mouse inputs, do not dispatch them to your main
  ...
  bool WantCaptureKeyboard; // When io.WantCaptureKeyboard is true, imgui will
                            ...
                            // your main game/application (in both cases, always
                            // pass keyboard inputs to imgui). (e.g. InputText
  ...
  bool NavVisible; // Directional navigation is visible and allowed (will handle
  ...
  ImVec2 MousePosPrev; // Previous mouse position temporary storage (nb: not for
                       // public use, set to MousePos in NewFrame())
...
public:
  ...
  inline void reserve(int new_capacity) {
    if (new_capacity <= Capacity)
      return;
    value_type *new_data = (value_type *)ImGui::MemAlloc((size_t)new_capacity *
                                                         sizeof(value_type));
    if (Data) {
...
struct ImDrawList {
  ...
  IMGUI_API void
  AddRect(const ImVec2 &a, const ImVec2 &b, ImU32 col, float rounding = 0.0f,
  ...
  IMGUI_API void
  AddRectFilled(const ImVec2 &a, const ImVec2 &b, ImU32 col,
                float rounding = 0.0f,
  ...
  IMGUI_API void
  AddConvexPolyFilled(const ImVec2 *points, const int num_points,
                      ImU32 col); // Note: Anti-aliased filling requires points
...
struct ImDrawData {
  ...
  ImVec2 DisplaySize; // Size of the viewport to render (== io.DisplaySize for
...
struct ImFontConfig {
  ...
  unsigned int
      RasterizerFlags; // 0x00     // Settings for custom font rasterizer (e.g.
...
struct ImFontAtlas {
  ...
  struct CustomRect {
    unsigned int ID; // Input    // User ID. Use <0x10000 to map into a font
                     // glyph, >=0x10000 for other/internal/custom texture data.
    unsigned short Width, Height; // Input    // Desired rectangle dimension
    unsigned short X, Y;          // Output   // Packed position in Atlas
    float GlyphAdvanceX;          // Input    // For custom font glyphs only
                                  // (ID<0x10000): glyph xadvance
    ImVec2 GlyphOffset; // Input    // For custom font glyphs only (ID<0x10000):
  ...
  AddCustomRectFontGlyph(ImFont *font, ImWchar id, int width, int height,
                         ...
                         const ImVec2 &offset = ImVec2(
                             0, 0)); // Id needs to be < 0x10000 to register a
