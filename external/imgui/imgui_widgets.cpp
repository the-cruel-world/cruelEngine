...
bool ImGui::ArrowButtonEx(const char *str_id, ImGuiDir dir, ImVec2 size,
  ...
  RenderArrow(
      bb.Min +
          ImVec2(ImMax(0.0f, size.x - g.FontSize - g.Style.FramePadding.x),
...
void ImGui::AlignTextToFramePadding() {
  ...
  ImGuiContext &g = *GImGui;
  window->DC.CurrentLineSize.y = ImMax(window->DC.CurrentLineSize.y,
                                       g.FontSize + g.Style.FramePadding.y * 2);
  window->DC.CurrentLineTextBaseOffset =
...
void ImGui::Separator() {
  ...
  ItemSize(ImVec2(
      0.0f,
      0.0f)); // NB: we don't provide our width so that it doesn't get feed back
              // into AutoFit, we don't provide height to not alter layout.
  if (!ItemAdd(bb, 0)) {
    if (window->DC.ColumnsSet)
      PushColumnClipRect();
    return;
...
static inline int DataTypeFormatString(char *buf, int buf_size,
  ...
  if (data_type == ImGuiDataType_S32 ||
      data_type == ImGuiDataType_U32) // Signedness doesn't matter when pushing
                                      // the argument
    return ImFormatString(buf, buf_size, format, *(const ImU32 *)data_ptr);
  if (data_type == ImGuiDataType_S64 ||
      data_type == ImGuiDataType_U64) // Signedness doesn't matter when pushing
                                      // the argument
...
static bool DataTypeApplyOpFromText(const char *buf,
  ...
  if (data_type == ImGuiDataType_S32) {
    ...
    // Assign constant
    // FIXME: We don't bother handling support for legacy operators since they
    // are a little too crappy. Instead we may implement a proper expression
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE *v, float v_speed,
  ...
  if (is_power) {
    ...
    v_cur = v_min + (TYPE)ImPow(ImSaturate((float)v_new_norm_curved), power) *
                        (v_max - v_min);
...
bool ImGui::DragScalar(const char *label, ImGuiDataType data_type, void *v,
  ...
  if (power != 1.0f)
    IM_ASSERT(v_min != NULL &&
              v_max != NULL); // When using a power curve the drag needs to have
                              // known bounds
  ...
  // Patch old "%.0f" format string to use "%d", read function comments for more
  // details.
...
bool ImGui::DragScalarN(const char *label, ImGuiDataType data_type, void *v,
  ...
  for (int i = 0; i < components; i++) {
    PushID(i);
    value_changed |=
        DragScalar("##v", data_type, v, v_speed, v_min, v_max, format, power);
...
bool ImGui::InputScalarAsWidgetReplacement(const ImRect &bb, ImGuiID id,
  ...
  bool value_changed =
      InputTextEx(label, data_buf, IM_ARRAYSIZE(data_buf), bb.GetSize(), flags);
  ...
  if (value_changed)
    return DataTypeApplyOpFromText(data_buf, g.InputTextState.InitialText.Data,
...
bool ImGui::InputScalar(const char *label, ImGuiDataType data_type,
                        ...
                        const char *format, ImGuiInputTextFlags extra_flags) {
  ImGuiWindow *window = GetCurrentWindow();
  if (window->SkipItems)
    return false;
  ...
  if (step != NULL) {
    ...
    if (InputText("", buf, IM_ARRAYSIZE(buf),
      ...
      value_changed = DataTypeApplyOpFromText(
          buf, g.InputTextState.InitialText.Data, data_type, data_ptr, format);
    PopItemWidth();
...
bool ImGui::InputScalarN(const char *label, ImGuiDataType data_type, void *v,
  ...
  for (int i = 0; i < components; i++) {
    PushID(i);
    value_changed |=
        InputScalar("##v", data_type, v, step, step_fast, format, extra_flags);
    SameLine(0, g.Style.ItemInnerSpacing.x);
    PopID();
    PopItemWidth();
    v = (void *)((char *)v + type_size);
...
bool ImGui::InputTextEx(const char *label, char *buf, int buf_size,
  ...
  if (focus_requested || user_clicked || user_scrolled ||
    ...
    if (g.ActiveId != id) {
      ...
      edit_state.TextW.resize(
          buf_size + 1); // wchar count <= UTF-8 count. we use +1 to make sure
                         // that .Data isn't NULL so it doesn't crash.
      edit_state.InitialText.resize(init_buf_len +
                                    1); // UTF-8. we use +1 to make sure that
                                        // .Data isn't NULL so it doesn't crash.
      memcpy(edit_state.InitialText.Data, buf, init_buf_len + 1);
      const char *buf_end = NULL;
      edit_state.CurLenW = ImTextStrFromUtf8(edit_state.TextW.Data, buf_size,
                                             buf, NULL, &buf_end);
      edit_state.CurLenA =
  ...
  if (g.ActiveId == id) {
    if (!is_editable && !g.ActiveIdIsJustActivated) {
      ...
      edit_state.CurLenW = ImTextStrFromUtf8(
          edit_state.TextW.Data, edit_state.TextW.Size, buf, NULL, &buf_end);
    ...
    if (io.InputCharacters[0]) {
      ...
      if (!ignore_inputs && is_editable && !user_nav_input_start)
        for (int n = 0;
             n < IM_ARRAYSIZE(io.InputCharacters) && io.InputCharacters[n];
             n++) {
  ...
  if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
    ...
    if (IsKeyPressedMap(ImGuiKey_LeftArrow)) {
      edit_state.OnKeyPressed((is_startend_key_down   ? STB_TEXTEDIT_K_LINESTART
                               : is_wordmove_key_down ? STB_TEXTEDIT_K_WORDLEFT
                                                      : STB_TEXTEDIT_K_LEFT) |
                              k_mask);
    } else if (IsKeyPressedMap(ImGuiKey_RightArrow)) {
      edit_state.OnKeyPressed((is_startend_key_down   ? STB_TEXTEDIT_K_LINEEND
      ...
      if (io.SetClipboardTextFn) {
        ...
        ImTextStrToUtf8(edit_state.TempBuffer.Data, edit_state.TempBuffer.Size,
      ...
      if (const char *clipboard = GetClipboardText()) {
        ...
        for (const char *s = clipboard; *s;) {
          ...
          if (c == 0)
            break;
          if (c >= 0x10000 || !InputTextFilterCharacter(&c, flags, callback,
                                                        callback_user_data))
            continue;
    ...
    if (apply_edit_back_to_user_buffer) {
      ...
      if ((flags & (ImGuiInputTextFlags_CallbackCompletion |
        ...
        if (event_flag) {
          ...
          IM_ASSERT(
              callback_data.Buf ==
              edit_state.TempBuffer.Data); // Invalid to modify those fields
...
bool ImGui::ColorButton(const char *desc_id, const ImVec4 &col,
  ...
  ImRect bb_inner = bb;
  float off =
      -0.75f; // The border (using Col_FrameBg) tends to look off when color is
              // near-opaque and rounding is enabled. This offset seemed like a
              // good middle ground to reduce those artifacts.
  bb_inner.Expand(off);
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags) {
  ...
  IM_ASSERT(ImIsPowerOfTwo(
      (int)(flags & ImGuiColorEditFlags__InputsMask))); // Check only 1 option
                                                        // is selected
  IM_ASSERT(ImIsPowerOfTwo(
      (int)(flags & ImGuiColorEditFlags__DataTypeMask))); // Check only 1 option
                                                          // is selected
  IM_ASSERT(ImIsPowerOfTwo(
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
  ...
  const float text_base_offset_y = ImMax(
      padding.y,
      window->DC.CurrentLineTextBaseOffset); // Latch before ItemSize changes it
  const float frame_height = ImMax(ImMin(window->DC.CurrentLineSize.y,
  ...
  // ItemSpacing (Ideally we'd want to add a flag for the user to specify if we
  // want the hit test to be done up to the right side of the content or not)
  ...
  if (!(flags & ImGuiTreeNodeFlags_Leaf)) {
    ...
    if (pressed) {
      ...
      if (g.DragDropActive &&
          is_open) // When using Drag and Drop "hold to open" we keep the node
  ...
  if (display_frame) {
    ...
    if (flags & ImGuiTreeNodeFlags_Bullet)
      ...
      RenderArrow(frame_bb.Min + ImVec2(padding.x, g.FontSize * 0.15f +
                                                       text_base_offset_y),
...
bool ImGui::CollapsingHeader(const char *label, bool *p_open,
  ...
  if (p_open) {
    ...
    ImVec2 button_center =
        ImVec2(ImMin(window->DC.LastItemRect.Max.x, window->ClipRect.Max.x) -
                   g.Style.FramePadding.x - button_radius,
               window->DC.LastItemRect.GetCenter().y);
    if (CloseButton(window->GetID((void *)(intptr_t)(id + 1)), button_center,
                    button_radius))
      *p_open = false;
...
bool ImGui::ListBoxHeader(const char *label, int items_count,
                          int height_in_items) {
  // Size default to hold ~7 items. Fractional number of items helps seeing that
  // we can scroll down/up without looking at scrollbar. We don't add +0.40f if
  // items_count <= height_in_items. It is slightly dodgy, because it means a
  // dynamic list of items will make the widget resize occasionally when it
  // crosses that size. I am expecting that someone will come and complain about
  // this behavior in a remote future, then we can advise on a better solution.
...
bool ImGui::BeginMenu(const char *label, bool enabled) {
  ...
  // a suitable position for the child menu (using FindBestWindowPosForPopup).
...
void ImGui::EndMenu() {
  ...
  // menu. A menu doesn't close itself because EndMenuBar() wants the catch the
  // last Left<>Right inputs. However, it means that with the current code, a
  // BeginMenu() from outside another menu or a menu-bar won't be closable with
  // the Left direction.
  ImGuiContext &g = *GImGui;
  ImGuiWindow *window = g.CurrentWindow;
