...
void ImGui::ShowDemoWindow(bool *p_open) {
  ...
  // ImGui::PushItemWidth(ImGui::GetWindowWidth() * 0.65f);    // Use 2/3 of the
  // space for widgets and 1/3 for labels (default)
  ...
  if (ImGui::CollapsingHeader("Help")) {
    ...
    ImGui::BulletText("Please see the ShowDemoWindow() code in imgui_demo.cpp. "
                      "<- you are here!");
  ...
  if (ImGui::CollapsingHeader("Configuration")) {
    ...
    if (ImGui::TreeNode("Configuration##2")) {
      ...
      ImGui::SameLine();
      ShowHelpMarker(
          "Required back-end to feed in gamepad inputs in io.NavInputs[] and "
          "set io.BackendFlags |= ImGuiBackendFlags_HasGamepad.\n\nRead "
          "instructions in imgui.cpp for details.");
      ImGui::CheckboxFlags("io.ConfigFlags: NavEnableSetMousePos",
                           (unsigned int *)&io.ConfigFlags,
                           ImGuiConfigFlags_NavEnableSetMousePos);
      ImGui::SameLine();
      ShowHelpMarker("Instruct navigation to move the mouse cursor. See "
      ...
      if (io.ConfigFlags &
          ImGuiConfigFlags_NoMouse) // Create a way to restore this flag
                                    // otherwise we could be stuck completely!
      {
      ...
      ImGui::SameLine();
      ShowHelpMarker("Set to false to disable blinking cursor, for users who "
                     "consider it distracting");
      ImGui::Checkbox("io.ConfigResizeWindowsFromEdges [beta]",
                      &io.ConfigResizeWindowsFromEdges);
      ImGui::SameLine();
      ShowHelpMarker("Enable resizing of windows from their edges and from the "
      ...
      ImGui::SameLine();
      ShowHelpMarker(
          "Instruct Dear ImGui to render a mouse cursor for you. Note that a "
    ...
    if (ImGui::TreeNode("Backend Flags")) {
      ImGuiBackendFlags backend_flags =
          io.BackendFlags; // Make a local copy to avoid modifying the back-end
                           // flags.
    ...
    if (ImGui::TreeNode("Capture/Logging")) {
      ImGui::TextWrapped("The logging API redirects all text output so you can "
                         ...
                         "Tree nodes can be automatically expanded.");
      ShowHelpMarker("Try opening any of the contents below in this window and "
                     "then click one of the \"Log To\" button.");
      ImGui::LogButtons();
  ...
  if (ImGui::CollapsingHeader("Widgets")) {
    if (ImGui::TreeNode("Basic")) {
...

      {
        ...
        ShowHelpMarker("Refer to the \"Combo\" section below for an "
                       "explanation of the full BeginCombo/EndCombo API, and "
...

      {
        ...
        ShowHelpMarker(
            ...
            "ESCAPE to revert.\n\nPROGRAMMER:\nYou can use the "
            "ImGuiInputTextFlags_CallbackResize facility if you need to wire "
            "InputText() to a dynamic string type. See misc/stl/imgui_stl.h "
            "for an example (this is not demonstrated in imgui_demo.cpp).");
...

      {
        ...
        ShowHelpMarker(
            ...
            "hold to use drag and drop.\nRight-click on the colored square to "
    ...
    if (ImGui::TreeNode("Trees")) {
      ...
      if (ImGui::TreeNode("Advanced, with Selectable nodes")) {
        ...
        ImGui::PushStyleVar(ImGuiStyleVar_IndentSpacing,
                            ImGui::GetFontSize() *
                                3); // Increase spacing to differentiate leaves
                                    // from expanded contents.
        for (int i = 0; i < 6; i++) {
    ...
    if (ImGui::TreeNode("Text")) {
      ...
      if (ImGui::TreeNode("Word Wrapping")) {
        ...
        ImGui::Text("The lazy dog is a good dog. This paragraph is made to fit "
      ...
      if (ImGui::TreeNode("UTF-8 Text")) {
        ...
        // signature')
        // - FOR THIS DEMO FILE ONLY, BECAUSE WE WANT TO SUPPORT OLD COMPILERS,
        // WE ARE *NOT* INCLUDING RAW UTF-8 CHARACTERS IN THIS SOURCE FILE.
        //   Instead we are encoding a few strings with hexadecimal constants.
        //   Don't do this in your application! Please use u8"text in any
        ...
        ImGui::TextWrapped(
            ...
            "manually to load extra character ranges. Read "
            "misc/fonts/README.txt for details.");
        ImGui::Text(
            "Hiragana: "
        ...
        // static char buf[32] = u8"NIHONGO"; // <- this is how you would write
        // it with C++11, using real kanjis
    ...
    if (ImGui::TreeNode("Images")) {
      ...
      // have access to inside the demo code. Remember that ImTextureID is just
      // storage for whatever you want it to be, it is essentially a value that
      // will be passed to the render function inside the ImDrawCmd structure.
      ...
      // be stored in ImTextureID. (for example, the imgui_impl_dx11.cpp
      // renderer expect a 'ID3D11ShaderResourceView*' pointer. The
      // imgui_impl_glfw_gl3.cpp renderer expect a GLuint OpenGL texture
      // identifier etc.) If you decided that ImTextureID = MyEngineTexture*,
      // then you can pass your MyEngineTexture* pointers to ImGui::Image(), and
      // gather width/height through your own functions, etc. Using
      // ShowMetricsWindow() as a "debugger" to inspect the draw data that are
      // being passed to your render will help you debug issues if you are
      // confused about this. Consider using the lower-level
      // ImDrawList::AddImage() API, via ImGui::GetWindowDrawList()->AddImage().
    ...
    if (ImGui::TreeNode("Selectables")) {
      ...
      // information (convenient in some cases) The earlier is more flexible, as
      // in real application your selection may be stored in a different manner
      // (in flags within objects, as an external list, etc).
      if (ImGui::TreeNode("Basic")) {
    ...
    if (ImGui::TreeNode("Multi-line Text Input")) {
      ...
      ShowHelpMarker("You can use the ImGuiInputTextFlags_CallbackResize "
                     "facility if you need to wire InputTextMultiline() to a "
                     "dynamic string type. See misc/stl/imgui_stl.h for an "
...
// natural type maximum, hence the /2 below.
      ...
      ImGui::SameLine();
      ShowHelpMarker(
          "As with every widgets in dear imgui, we never modify values unless "
          "there is a user interaction.\nYou can override the clamping limits "
          "by using CTRL+Click to input a value.");
      ...
      ImGui::DragScalar("drag double ^2", ImGuiDataType_Double, &f64_v, 0.0005f,
                        &f64_zero, &f64_one, "0 < %.10f < 1", 2.0f);
    ...
    if (ImGui::TreeNode("Drag and Drop")) {
      {
        ...
        // They are using standardized payload strings
        // IMGUI_PAYLOAD_TYPE_COLOR_3F and IMGUI_PAYLOAD_TYPE_COLOR_4F to allow
        // your own widgets to use colors in their drag and drop interaction.
        // Also see the demo in Color Picker -> Palette demo.
        ...
        for (int n = 0; n < IM_ARRAYSIZE(names); n++) {
          ...
          if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None)) {
            ImGui::SetDragDropPayload(
                "DND_DEMO_CELL", &n,
                sizeof(int)); // Set payload to carry the index of our item
                              // (could be anything)
            if (mode == Mode_Copy) {
              ImGui::Text("Copy %s", names[n]);
            } // Display preview (could be anything, e.g. when dragging an image
              // we could decide to display the filename and a small preview of
              // the image, etc.)
            if (mode == Mode_Move) {
    ...
    if (ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)")) {
      ...
      // functions. Note that the flags can be combined. (because BulletText is
      // an item itself and that would affect the output of IsItemHovered() we
      // pass all state in a single call to simplify the code).
      ...
      if (embed_all_inside_a_child_window)
        ImGui::BeginChild("outer_child", ImVec2(0, ImGui::GetFontSize() * 20),
                          true);
      ...
      ImGui::Text("This is another child window for testing with the "
                  "_ChildWindows flag.");
      ImGui::EndChild();
      if (embed_all_inside_a_child_window)
        EndChild();
      ...
      // menu (with BeginPopupContextItem) associated to the title bar of a
      // window.
  ...
  if (ImGui::CollapsingHeader("Layout")) {
    ...
    if (ImGui::TreeNode("Text Baseline Alignment")) {
      ...
      ImGui::AlignTextToFramePadding(); // If your line starts with text, call
                                        // this to align it to upcoming widgets.
      ...
      ImGui::AlignTextToFramePadding(); // Vertically align text node a bit
                                        // lower so it'll be vertically centered
                                        // with upcoming widget. Otherwise you
    ...
    if (ImGui::TreeNode("Scrolling")) {
      ...
      ImGui::SameLine(130);
      scroll_to |=
          ImGui::DragInt("##pos_y", &scroll_to_px, 1.00f, 0, 9999, "Y = %d px");
      ImGui::PopItemWidth();
      if (scroll_to)
        track = false;
  ...
  if (ImGui::CollapsingHeader("Popups & Modal windows")) {
    ...
    if (ImGui::TreeNode("Context menus")) {
      ...
      ImGui::Button(buf);
      if (ImGui::BeginPopupContextItem()) // When used after an item that has an
                                          // ID (here the Button), we can skip
                                          // providing an ID to
    ...
    if (ImGui::TreeNode("Modals")) {
      ...
      if (ImGui::BeginPopupModal("Stacked 1")) {
        ...
        ImGui::ColorEdit4("color",
                          color); // This is to test behavior of stacked regular
                                  // popups over a modal
  ...
  if (ImGui::CollapsingHeader("Inputs, Navigation & Focus")) {
    ...
    if (ImGui::TreeNode("Dragging")) {
      ...
      if (ImGui::IsItemActive()) {
        ...
        // io.MouseDragThreshold) You can request a lower or higher threshold
        // using the second parameter of IsMouseDragging() and
        // GetMouseDragDelta()
...
void ImGui::ShowStyleEditor(ImGuiStyle *ref) {
  ...
  if (ImGui::TreeNode("Rendering")) {
    ...
    ImGui::SameLine();
    ShowHelpMarker("When disabling anti-aliasing lines, you'll probably want "
                   "to disable borders in your style as well.");
  ...
  if (ImGui::TreeNode("Settings")) {
    ...
    ImGui::SameLine();
    ShowHelpMarker("Adjust if you cannot see the edges of your screen (e.g. on "
                   "a TV where scaling has not been configured).");
  ...
  if (ImGui::TreeNode("Colors")) {
    ...
    for (int i = 0; i < ImGuiCol_COUNT; i++) {
      ...
      if (memcmp(&style.Colors[i], &ref->Colors[i], sizeof(ImVec4)) != 0) {
        ...
        // icons. Read the FAQ and misc/fonts/README.txt about using icon fonts.
...
struct ExampleAppConsole {
  ...
  static int
  TextEditCallbackStub(ImGuiInputTextCallbackData
...
static void ShowExampleAppCustomRendering(bool *p_open) {
  ...
  ImGui::ColorEdit3("Color", &col.x);
  {
    ...
    for (int n = 0; n < 2; n++) {
      ...
      draw_list->AddCircle(ImVec2(x + sz * 0.5f, y + sz * 0.5f), sz * 0.5f,
                           col32, 20, curr_thickness);
      x += sz + spacing;
      draw_list->AddRect(ImVec2(x, y), ImVec2(x + sz, y + sz), col32, 0.0f,
                         ImDrawCornerFlags_All, curr_thickness);
      x += sz + spacing;
    ...
    draw_list->AddRectFilled(ImVec2(x, y), ImVec2(x + sz, y + thickness),
                             col32);
    x += sz + spacing; // Horizontal line (faster than AddLine, but only handle
                       // integer thickness)
  ...
  ImGui::Separator();
  {
    ...
    // cursor and 2) allows us to use IsItemHovered() But you can also draw
