...
enum ImGuiNavMoveFlags_ {
  ...
  ImGuiNavMoveFlags_AlsoScoreVisibleSet =
      1 << 5 // Store alternate result in NavMoveResultLocalVisibleSet that only
...
struct ImGuiNextWindowData {
  ...
  float BgAlphaVal;
  ImVec2 MenuBarOffsetMinVal; // This is not exposed publicly, so we don't clear
                              // it.
...
struct ImGuiContext {
  ...
  ImGuiWindow *
      NavWindow; // Focused window for navigation. Could be called 'FocusWindow'
  ...
  ImGuiNavForward NavMoveRequestForward; // None / ForwardQueued / ForwardActive
                                         // (this is used to navigate sibling
  ...
  // Render
  ImDrawData DrawData; // Main ImDrawData instance to pass render information to
                       // the user
...
struct IMGUI_API ImGuiWindowTempData {
  ...
  ImVec2 CursorMaxPos; // Used to implicitly calculate the size of our contents,
...
struct IMGUI_API ImGuiWindow {
  ...
  ImVec2 SizeFullAtLastBegin; // Copy of SizeFull at the end of Begin. This is
                              // the reference value we'll use on the next frame
                              // to decide if we need scrollbars.
  ...
  int BeginOrderWithinParent; // Order within immediate parent window, if we are
                              // a child window. Otherwise 0.
  ...
  ImVec2 SetWindowPosVal; // store window position when using a non-zero Pivot
                          ...
                          // the window size)
  ImVec2
      SetWindowPosPivot; // store window pivot for positioning. ImVec2(0,0) when
                         // positioning from top-left corner; ImVec2(0.5f,0.5f)
                         // for centering; ImVec2(1,1) for bottom right.
  ...
  ImRect
      ContentsRegionRect; // FIXME: This is currently confusing/misleading.
                          ...
                          // (SizeContentsExplicit ? SizeContentsExplicit : Size
                          // - ScrollbarSizes) - CursorStartPos, per axis
...
IMGUI_API void
FocusFrontMostActiveWindowIgnoringOne(ImGuiWindow *ignore_window);
...
IMGUI_API void
RenderMouseCursor(ImDrawList *draw_list, ImVec2 pos, float scale,
